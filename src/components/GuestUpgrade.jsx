...
/**
 * Upgrade a guest session to a full account via magic code.
 * Instant keeps the same user id when the email is new.
 */
export function GuestUpgradeDialog({ open, onOpenChange }) {
            ...
            {!sentEmail ? (
              <>
                <p {...stylex.props(menus.mcap, menus.mcapFirst)}>
                  게스트로 만든 보드와 일정이 이 계정에 이어집니다. Instant Guest Auth가 같은
                  사용자 id를 유지합니다.
                </p>
                {err && (
