...
| --- | --- | --- |
| `INSTANT_APP_ID` | Instant CLI, Vercel APIs | Same value as `src/lib/config.js` → `APP_ID`. Also accepted as `VITE_INSTANT_APP_ID` in API routes. |
...
- **Browser** Instant client reads `APP_ID` from `src/lib/config.js` (not Vite `import.meta.env`).
