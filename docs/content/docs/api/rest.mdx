...
shown exactly once at creation/rotation (`src/api-tokens.js`).
...
grid and get snapped/clamped exactly like in-app edits (`src/models.js`).
...
1. **API layer** — 120 requests/minute per token (`src/rest.js`
